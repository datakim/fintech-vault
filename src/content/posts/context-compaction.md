...
설령 window 안에 들어가더라도 문제다. 쓸데없는 로그가 잔뜩 끼면 모델이 정작 중요한 정보에 집중하지 못하고, 비용과 응답 속도도 같이 나빠짐.

그래서 필요한 게 컨텍스트 압축(context compaction)이다. 핵심은 이것:

> 다음 판단에 필요한 것만 남기고, 나머지는 버린다.

참고로 이건 "프롬프트 압축(prompt compression)"과는 다름. 프롬프트 압축은 문장을 토큰 단위로 줄이는 기법(예: LLMLingua)에 가깝고, 여기서 말하는 건 에이전트의 대화·작업 상태를 관리하는 컨텍스트 압축이다.

가장 단순한 형태는 시스템 프롬프트와 사용자 요청은 유지하고, 중간 대화는 버리고, 최근 메시지 몇 개만 남기는 방식이다.
...
가장 단순한 방식이다. 예를 들어 에이전트가 tool call을 20번 했으면, 전부 넣지 않고 최근 5개만 유지함.
...
sliding window보다 훨씬 실무적이다. 무엇을 했고 무엇이 실패했는지가 살아남기 때문이다.
...
실무에서 context를 가장 많이 잡아먹는 건 보통 tool 결과다. SQL 결과, PDF 검색 결과, 브라우저 DOM, API 응답 같은 것들이다.
...
에이전트에게 필요한 건 raw result 전체가 아니라, 다음 판단에 쓸 관찰값이다.
...
압축에서 중요한 건 무조건 줄이는 게 아니다. 버려도 되는 것과 버리면 안 되는 것을 구분하는 거다.
...
조금 더 좋은 구조는 대화 대신 상태를 통째로 들고 다니는 방식이다.
...
컨텍스트 압축은 단순히 "짧게 줄이기"가 아니다. 정확히는 이렇다:
...
raw 대화 50턴을 그대로 넘기는 대신 이 상태판 하나를 넘기는 것 — 이게 컨텍스트 압축이다.
